jQuery(document).ready(function($) {
    ...
    jQuery('.upme-registration').find('#reg_user_login').bind('change keydown keyup',function(){
        if(UPMECustom.RegFormTitleUsername == '1'){
            let value = jQuery('#reg_user_login').val();
            jQuery('.upme-registration .upme-name .upme-field-name').text( value.replace(/[^a-zA-Z0-9]/g, "") );
        }
    });
...
// jQuery(document).on("keyup change", 'input[type=text] textarea select', function(e) {
